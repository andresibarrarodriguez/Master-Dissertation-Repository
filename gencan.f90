module bmgencan
  ...
  interface
     ...
     subroutine hsub(n,x,lim,hnnz,hrow,hcol,hval,ierr,pdata)
       use iso_c_binding, only: c_ptr
       implicit none
     ...
     subroutine stpsub(n,x,gsupn,iter,inhdefstp,stp,ierr,pdata)
       use iso_c_binding, only: c_ptr
       implicit none
     ...
     subroutine tabsub(n,f,gsupn,iter,pdata)
       use iso_c_binding, only: c_ptr
       implicit none
       integer, intent(in) :: iter,n
       real(kind=8), intent(in) :: f,gsupn
       type(c_ptr), optional, intent(in) :: pdata
  ...
  real(kind=8), parameter :: lrgstreal = huge( 1.0d0 ), macheps = epsilon( 1.0d0 ), &
       macheps12 = sqrt( macheps ), macheps14 = sqrt( macheps12 ), &
       gamma = 1.0d-04, lspgmin = 1.0d-16, lspgmax = 1.0d+16, &
...
contains
  ...
  subroutine genunc(evalf,evalg,evalh,hnnzmax,hfixstr,n,x,f,g,gsupn, &
    ...
    integer, intent(in) :: hnnzmax,maxit,n,nfnoprogrmax
    integer, intent(out) :: ierr,istop,iter
    real(kind=8), intent(in) :: beta,eta,ftarget,eps,sgmmult
    real(kind=8), intent(out) :: f,gsupn
    type(c_ptr), optional, intent(in) :: pdata

    ! ARRAY ARGUMENTS
    integer, intent(in) :: maxcountg(4)
    ...
    call genuncb(evalf,evalg,evalh,hfixstr,n,x,f,g,gsupn,lss,ftarget,eps, &
         maxit,extallowed,nfnoprogrmax,beta,eta,sgmmult,maxcountg,iter,ierr, &
         istop,evalstp,tabline,pdata)
  ...
  subroutine genuncb(evalf,evalg,evalh,hfixstr,n,x,f,g,gsupn,lss,ftarget, &
       eps,maxit,extallowed,nfnoprogrmax,beta,eta,sgmmult,maxcountg,iter, &
       ierr,istop,evalstp,tabline,pdata)
    ...
    ! SCALAR ARGUMENTS
    logical, intent(in) :: extallowed,hfixstr
    integer, intent(in) :: maxit,n,nfnoprogrmax
    integer, intent(inout) :: ierr
...
100 continue
    ...
    if ( present( tabline ) ) then
       call tabline(n,f,gsupn,iter,pdata)
    end if
    
    inhdefstp = .false.
    
    if ( present( evalstp ) ) then
       call evalstp(n,x,gsupn,iter,inhdefstp,ustp,ierr,pdata)
       if ( ierr .ne. 0 ) return
    ...
    if( .not. inhdefstp )  then
       if ( countg(1) .ge. maxcountg(1) ) then
          write(*,*) 'Stopping criterion gsupn <= eps satisfied'
          istop = 0
          return
       else if ( countg(2) .ge. maxcountg(2) ) then
          write(*,*) 'Stopping because gsupn <= eps^1/2 during ',maxcountg(2),' consecutive iterations'
          istop = 1
          return
       else if ( countg(3) .ge. maxcountg(3) ) then
          write(*,*) 'Stopping because gsupn <= eps^1/4 during ',maxcountg(3),' consecutive iterations'
          istop = 2
          return
       else if ( countg(4) .ge. maxcountg(4) ) then
          write(*,*) 'Stopping because gsupn <= eps^1/8 during ',maxcountg(4),' consecutive iterations'
          istop = 3
          return
       end if
       ...
       if ( f .lt. fbest ) then
          ...
          if ( nfnoprogr .gt. nfnoprogrmax ) then
...
210 continue
    ...
    if ( nneigv .ne. dim - n .or. rankdef ) then
       ...
       if ( sgmmult * sgmtrial .le. lrgstreal ) then
          sgmtrial = sgmmult * sgmtrial
          go to 210
       else
...
410          continue
             ...
             if ( tex .le. texmax ) then
                if ( .not. fref .le. ftarget ) then
                   xext(1:n) = x(1:n) + 2.0d0 ** tex * d(1:n)
  ...
  subroutine gencan(evalf,evalg,evalh,hnnzmax,hfixstr,n,x,lind, &
       lbnd,uind,ubnd,f,g,gpsupn,ftarget,eps,maxit,extallowed, &
       nfnoprogrmax,beta,eta,r,sgmmult,maxcountg,iter,ierr,istop, &
       evalstp,tabline,pdata)
    ...
    integer, intent(in) :: hnnzmax,maxit,n,nfnoprogrmax
    integer, intent(out) :: ierr,istop,iter
    real(kind=8), intent(in) :: beta,eta,ftarget,eps,r,sgmmult
    ...
    call gencanb(evalf,evalg,evalh,hfixstr,n,x,lind,lbnd,uind,ubnd, &
         f,g,gpsupn,lss,ftarget,eps,maxit,extallowed,nfnoprogrmax,beta, &
         eta,r,sgmmult,maxcountg,iter,ierr,istop,evalstp,tabline,pdata)
  ...
  subroutine gencanb(evalf,evalg,evalh,hfixstr,n,x,lind,lbnd,uind,ubnd, &
       f,g,gpsupn,lss,ftarget,eps,maxit,extallowed,nfnoprogrmax,beta,eta, &
       r,sgmmult,maxcountg,iter,ierr,istop,evalstp,tabline,pdata)
    ...
    real(kind=8) :: eps12,eps14,eps18,fbest,fbtrial,fext,fref,ftrial,gisupn,gtd, &
         gpsupnb,lspg,sts,sty,sgmini,sgmtrial,t,tref,ttmp
...
100 continue
    ...
    if ( present( tabline ) ) then
       call tabline(n,f,gpsupn,iter,pdata)
    end if

    inhdefstp = .false.
    
    if ( present( evalstp ) ) then
       call evalstp(n,x,gpsupn,iter,inhdefstp,ustp,ierr,pdata)
       if ( ierr .ne. 0 ) return
    ...
    if ( .not. inhdefstp ) then
       if ( countg(1) .ge. maxcountg(1) ) then
          ...
          write(*,*) 'Stopping because gpsupn <= eps^1/2 during ',maxcountg(2),' consecutive iterations'
          istop = 1
          return
       else if ( countg(3) .ge. maxcountg(3) ) then
          write(*,*) 'Stopping because gpsupn <= eps^1/4 during ',maxcountg(3),' consecutive iterations'
          istop = 2
          return
       else if ( countg(4) .ge. maxcountg(4) ) then
          write(*,*) 'Stopping because gpsupn <= eps^1/8 during ',maxcountg(4),' consecutive iterations'
          istop = 3
          return
       end if
...
310          continue
             ...
             if ( tex .le. texmax ) then
                if ( .not. fref .le. ftarget ) then
                   xext(ifr(1:nfr)) = x(ifr(1:nfr)) + 2.0d0 ** tex * tref * d(1:nfr)
