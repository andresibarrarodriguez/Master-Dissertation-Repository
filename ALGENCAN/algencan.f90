module bmalgencan
  ...
  use bmgencan, only: gencanb,genuncb,project
  ...
  interface
     ...
     subroutine tabsub(n,x,m,p,lambda,f,csupn,ssupn,nlpsupn,bdsvio,xtype,outiter,totiter, &
       nwcalls,nwtotit,pdata)
       use iso_c_binding, only: c_ptr
       implicit none
       integer, intent(in) :: m,n,nwcalls,nwtotit,outiter,p,totiter,xtype
       real(kind=8), intent(in) :: bdsvio,csupn,f,nlpsupn,ssupn
       real(kind=8), intent(in) :: lambda(m+p),x(n)
       type(c_ptr), optional, intent(in) :: pdata
  ...
  real(kind=8), parameter :: lrgstreal = huge( 1.0d0 ), macheps = epsilon( 1.0d0 ), &
      ...
      rhoinimin = macheps12, rhoinimax = macheps12inv
...
contains
  ...
  subroutine algencan(evalf,evalg,evalc,evalj,evalhl,jnnzmax,hlnnzmax, &
       ...
       scale,rhoauto,rhoini,corrin,f,csupn,ssupn,nlpsupn, &
       bdsvio,xtype,outiter,totiter,nwcalls,nwtotit,ierr,istop,tabline,pdata, rhomult, rhofrac)
    ...
    procedure(tabsub), optional :: tabline

    ! SCALAR ARGUMENTS
    logical, intent(in) :: corrin,rhoauto,scale
    integer, intent(in) :: hlnnzmax,jnnzmax,m,maxoutit,n,p
    integer, intent(out) :: ierr,istop,nwcalls,nwtotit,outiter,totiter,xtype
    real(kind=8), intent(in) :: epsfeas,epscompl,epsopt,rhoini, rhomult, rhofrac
    ...
    ! Note: in cases 1, 2, 3, and 4, if a KKT point was found then it
    ! was found by the acceleration process when the AL iterates were
    ! far from satisfying KKT conditions (less than half of the
    ! required precision). In this case, since stopping returning this
    ! KKT point was considered premature, the minimization process
    ! continued. At the end, it would have been more efficient to stop
    ! returning it, but how could we know it in advance?
    ...
    ! istop = 5 means the squared infeasibility is in fact
    ! minimized. Whether the final point is feasible or not depends on
    ! the reported feasibility, but istop = 5 in any case. The
    ! objective function at the final iterate of the
    ! squared-feasibility minimization process is evaluated and
    ! reported.

    ! Regardless of the criteria that made algencan stop, according to
    ! an internal criterion, Algencan returns, among all points
    ! generated by it, the one it considers to be the best.
    
    ! xtype = 0 means that the point Algencan returns corresponds to
    ! the intial given point.
    
    ! xtype = 1 means that the point Algencan returns corresponds to a
    ! point obtained through the acceleration process.
    
    ! xtype = 2 means that the point Algencan returns corresponds to
    ! an iterand of the augmented Lagrangian method.
    
    ! xtype = 3 means that the point Algencan returns corresponds to
    ! the minimization of the squared infeasibility. (xtype = 3 can
    ! only be associated with istop = 5.)
    
    ! LOCAL SCALARS
    logical :: extallowed,kktfound,feasfound
    integer :: bestxtype,iter,maxit,nbds
    ...
    call algencanb(lss,memev,memevb,n,x,lind,lbnd,uind,ubnd,nbds,m,p,lambda,epsfeas, &
         epscompl,epsopt,maxoutit,rhoauto,rhoini,corrin,f,csupn,ssupn,nlpsupn, &
         bdsvio,kktfound,feasfound,bestx,bestlambda,bestf,bestcsupn,bestssupn,bestnlpsupn, &
         bestbdsvio,bestxtype,outiter,totiter,nwcalls,nwtotit,ierr,istop,tabline, rhomult, rhofrac)
  ...
  subroutine algencanb(lss,memev,memevb,n,x,lind,lbnd,uind,ubnd,nbds,m,p,lambda,epsfeas, &
       epscompl,epsopt,maxoutit,rhoauto,rhoini,corrin,f,csupn,ssupn,nlpsupn, &
       bdsvio,kktfound,feasfound,bestx,bestlambda,bestf,bestcsupn,bestssupn,bestnlpsupn, &
       bestbdsvio,bestxtype,outiter,totiter,nwcalls,nwtotit,ierr,istop,tabline, rhomult, rhofrac)
    ...
    ! SCALAR ARGUMENTS
    logical, intent(in) :: corrin,rhoauto
    logical, intent(out) :: kktfound,feasfound
    integer, intent(in) :: m,maxoutit,n,nbds,p
    integer, intent(inout) :: ierr
    integer, intent(out) :: bestxtype,istop,nwcalls,nwtotit,outiter,totiter
    ...
    real(kind=8), intent(in), target :: lbnd(n),ubnd(n)
    ...
    ! LOCAL SCALARS
    logical :: hfixstr,infeasfound,nwfeasfound,nwinfeasfound,restart
    integer :: a,b,cnoprog,feasxtype,infeasxtype,innistop,inniter,j,kktxtype,maxit,nwierr, &
         nwistop,nwiter,nwmaxit,subfail,nrestart
    real(kind=8) :: al,csupnprev,ftarget,nfeaspsupn,nwbdsvio,nwcsupn,nwnlpsupn, &
    ...
    real(kind=8) :: feaslambda(m+p),feasx(n),infeaslambda(m+p),infeasx(n),kktlambda(m+p), &
         kktx(n),nfeas(n),nl(n),nlp(n),nwx(n),nwl(m+p),nwfeasx(n),nwfeasl(m+p),nwinfeasx(n), &
         nwinfeasl(m+p)
    ...
    nlp(1:n) = x(1:n) - nl(1:n)

    call project(n,lind,lbnd,uind,ubnd,nlp)

    nlp(1:n) = nlp(1:n) - x(1:n)
    
    nlpsupn = maxval( abs ( nlp(1:n) ) )
    ...
    call updatesol(epsfeas,n,x,m,p,lambda,f,csupn,bdsvio,ssupn,nlpsupn,0, &
         kktfound,kktxtype,kktf,kktx,kktlambda,kktcsupn,kktbdsvio,kktssupn,kktnlpsupn, &
         feasfound,feasxtype,feasf,feasx,feaslambda,feascsupn,feasbdsvio,feasssupn,feasnlpsupn, &
         infeasfound,infeasxtype,infeasf,infeasx,infeaslambda,infeascsupn,infeasbdsvio,infeasssupn,infeasnlpsupn, &
         bestxtype,bestf,bestx,bestlambda,bestcsupn,bestbdsvio,bestssupn,bestnlpsupn, &
         epscompl,epsopt,outiter,totiter,nwcalls,nwtotit,memev,tabline)
    if ( ierr .ne. 0 ) return

    write(*,*) 'kkt:    ',kktfound,kktxtype,kktf,kktcsupn,kktbdsvio,kktssupn,kktnlpsupn
    write(*,*) 'feas:   ',feasfound,feasxtype,feasf,feascsupn,feasbdsvio,feasssupn,feasnlpsupn
    write(*,*) 'infeas: ',infeasfound,infeasxtype,infeasf,infeascsupn,infeasbdsvio,infeasssupn,infeasnlpsupn
    write(*,*) 'best:   ',bestxtype,bestf,bestcsupn,bestbdsvio,bestssupn,bestnlpsupn
...
100 continue
    ...
    if ( nwfeasfound ) then
       ...
       call updatesol(epsfeas,n,nwfeasx,m,p,nwfeasl,nwfeasf,nwfeascsupn,nwfeasbdsvio,nwfeasssupn, &
            nwfeasnlpsupn,1,kktfound,kktxtype,kktf,kktx,kktlambda,kktcsupn,kktbdsvio,kktssupn,kktnlpsupn, &
            feasfound,feasxtype,feasf,feasx,feaslambda,feascsupn,feasbdsvio,feasssupn,feasnlpsupn, &
            infeasfound,infeasxtype,infeasf,infeasx,infeaslambda,infeascsupn,infeasbdsvio,infeasssupn,infeasnlpsupn, &
            bestxtype,bestf,bestx,bestlambda,bestcsupn,bestbdsvio,bestssupn,bestnlpsupn, &
            epscompl,epsopt,outiter,totiter,nwcalls,nwtotit,memev,tabline)
       ...
       call updatesol(epsfeas,n,nwinfeasx,m,p,nwinfeasl,nwinfeasf,nwinfeascsupn,nwinfeasbdsvio,nwinfeasssupn, &
            nwinfeasnlpsupn,1,kktfound,kktxtype,kktf,kktx,kktlambda,kktcsupn,kktbdsvio,kktssupn,kktnlpsupn, &
            feasfound,feasxtype,feasf,feasx,feaslambda,feascsupn,feasbdsvio,feasssupn,feasnlpsupn, &
            infeasfound,infeasxtype,infeasf,infeasx,infeaslambda,infeascsupn,infeasbdsvio,infeasssupn,infeasnlpsupn, &
            bestxtype,bestf,bestx,bestlambda,bestcsupn,bestbdsvio,bestssupn,bestnlpsupn, &
            epscompl,epsopt,outiter,totiter,nwcalls,nwtotit,memev,tabline)
    ...
    restart = .false.
!!$    if ( subfail .gt. 0 .and. mod(subfail,3) .eq. 0 .and csupn .le. epsfeas ) then
!!$       restart = .true.
!!$       nrestart = nrestart + 1
!!$       write(*,*) 'The penalty parameter will be restarted.'
!!$
!!$       if ( nrestart .gt. 5 ) then
!!$          write(*,*) 'Too many restarts of the penalty parameter.'
!!$          istop = 2
!!$          return
!!$       end if
!!$    end if
    
    if ( outiter .eq. 1 .or. restart ) then
       if ( rhoauto ) then
          call memevalf(n,x,ierr,memev)
          if ( ierr .ne. 0 ) return
...
!!$       if ( .not. ( innistop .eq. 0 .and. inniter .eq. 0 ) .and. &
!!$            ( ( csupn .gt. epsfeas  .and. .not. csupn .le. rhofrac * csupnprev ) .or. &
!!$            ( ssupn .gt. epscompl .and. .not. ssupn .le. rhofrac * ssupnprev ) ) ) then
!!$          rho = rhomult * rho
!!$       end if
    end if
    ...
    if ( nbds .eq. 0 ) then
       call genuncb(evalal,evalnal,evalhal,hfixstr,n,x,al,nl,nlpsupn,lss,ftarget, &
            epsopk,maxit,.false.,inniter,ierr,innistop,pdata=c_loc(aldata))
    else
       call gencanb(evalal,evalnal,evalhal,hfixstr,n,x,lind,lbnd,uind,ubnd, &
            al,nl,nlpsupn,lss,ftarget,epsopk,maxit,.false.,inniter,ierr, &
    ...
    if ( max( csupn, bdsvio ) .le. epsfeas ) then
       ...
       call updatesol(epsfeas,n,x,m,p,lambda,f,csupn,bdsvio,ssupn,nlpsupn,2, &
            kktfound,kktxtype,kktf,kktx,kktlambda,kktcsupn,kktbdsvio,kktssupn,kktnlpsupn, &
            feasfound,feasxtype,feasf,feasx,feaslambda,feascsupn,feasbdsvio,feasssupn,feasnlpsupn, &
            infeasfound,infeasxtype,infeasf,infeasx,infeaslambda,infeascsupn,infeasbdsvio,infeasssupn,infeasnlpsupn, &
            bestxtype,bestf,bestx,bestlambda,bestcsupn,bestbdsvio,bestssupn,bestnlpsupn, &
    ...
    if ( csupn .le. epsfeas .or. outiter .eq. 1 .or. csupn .lt. csupnprev ) then
       ...
       write(*,*) 'Moreover, in the current iteration there was not even simple decrease in the feasibility.'
       write(*,*) 'This is the sup-norm of the continuous projected gradient of 1/2 of the squared ', &
...
!!$    end if
    ...
    if ( csupn .le. epsfeas ) then
       if ( innistop .eq. 0 ) then
          ...
          if ( subfail .ge. 3 ) then
             write(*,*) 'Too many consecutive fails of the inner solver; and the current point is feasible.'
             istop = 2
             return
          end if
       end if
  ...
  subroutine updatesol(epsfeas,n,x,m,p,lambda,f,csupn,bdsvio,ssupn, &
       nlpsupn,xtype, kktfound,kktxtype,kktf,kktx,kktlambda, &
       kktcsupn,kktbdsvio,kktssupn,kktnlpsupn, feasfound,feasxtype, &
       feasf,feasx,feaslambda,feascsupn,feasbdsvio,feasssupn, &
       feasnlpsupn, infeasfound,infeasxtype,infeasf,infeasx, &
       infeaslambda,infeascsupn,infeasbdsvio,infeasssupn, &
       infeasnlpsupn, bestxtype,bestf,bestx,bestlambda,bestcsupn, &
       bestbdsvio,bestssupn,bestnlpsupn, epscompl,epsopt,outiter, &
       totiter,nwcalls,nwtotit,memev,tabline)
    ...
    integer, intent(in) :: m,n,p,outiter,totiter,xtype,nwcalls,nwtotit
    integer, intent(inout) :: kktxtype,feasxtype,infeasxtype,bestxtype
    ...
    ! ARRAY ARGUMENTS
    real(kind=8), intent(in) :: lambda(m+p),x(n)
    real(kind=8), intent(inout) :: kktx(n),kktlambda(m+p),feasx(n),feaslambda(m+p), &
         infeasx(n),infeaslambda(m+p),bestx(n),bestlambda(m+p)
    ...
    if ( present( tabline ) ) then
       if ( memev%pdatapresent) then
          call tabline(n,bestx,m,p,bestlambda,bestf,bestcsupn,bestssupn,bestnlpsupn,bestbdsvio, &
               bestxtype,outiter,totiter,nwcalls,nwtotit,memev%pdata)
       else
          call tabline(n,bestx,m,p,bestlambda,bestf,bestcsupn,bestssupn,bestnlpsupn,bestbdsvio, &
               bestxtype,outiter,totiter,nwcalls,nwtotit)
       end if
    end if
