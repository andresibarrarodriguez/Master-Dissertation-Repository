...
program algencama
  ...
  type :: pdata_type
     ...
     integer :: m_orig,n_orig,nfree,nbds
  ...
  ! LOCAL SCALARS
  logical :: corrin,rhoauto,scale
  integer :: allocerr,e_order,hlnnzmax,i,ierr,istop,j,jnnz,jnnzmax,jfun(1),jvar(1),l_order, &
       v_order,m,maxoutit,n,nargs, ncdouble,nwcalls,nwtotit,outiter,p,status,totiter,xtype
  ...
  if ( pdata%m_orig .eq. 0 ) then
     write(*,*) 'The problem has no constraints. Use Gencan instead of Algencan.'
     stop
  end if
  ...
  pdata%cb(pdata%m_orig+1:m+p) = pack( clbnd(m+1:pdata%m_orig), cdouble(m+1:pdata%m_orig) )
  ...
  call algencan(evalf,evalg,evalc,evalj,evalhl,jnnzmax,hlnnzmax, &
       ...
       scale,rhoauto,rhoini,corrin,f,csupn,ssupn,nlpsupn,bdsvio, &
       xtype,outiter,totiter,nwcalls,nwtotit,ierr,istop,tabline,c_loc(pdata), rhomult, rhofrac)
  ...
  write(*,*) '(CUTEst formulation) Number of constraints            = ',pdata%m_orig
  write(*,*) '(CUTEst formulation) Number of equalities             = ',m
  write(*,*) '(CUTEst formulation) Number of one-sided inequalities = ',pdata%m_orig - m - ncdouble
  write(*,*) '(CUTEst formulation) Number of two-sided inequalities = ',ncdouble
  
  write(*,*)
  write(*,*) '(Problem effectively tackled) Number of variables     = ',n
  write(*,*) '(Problem effectively tackled) Number of equalities    = ',m
  write(*,*) '(Problem effectively tackled) Number of inequalities  = ',p
  ...
  write(*,*) '(REPORTED BY SOLVER) nature of the returned point     = ',xtype  
  ...
  write(20,9000) istop,ierr,finish-start,pdata%n_orig,pdata%m_orig,m,pdata%m_orig-m-ncdouble,ncdouble, &
       n,m,p,pdata%nbds,f,csupn,ssupn,nlpsupn,bdsvio,xtype,maxval(abs(x(1:n))),maxval(abs(lambda(1:m+p))), &
...
9000 format(1X,I2,1X,I3,0P,F12.6,9(1X,I6),1X,1P,D24.16,4(1X,1P,D7.1),1X,I1,2(1X,1P,D7.1),9(1X,I8))
...
contains
  ...
  subroutine evalj(n,x,m,p,ind,sorted,jsta,jlen,lim,jvar,jval,inform,pdataptr)
    ...
    ! LOCAL SCALARS
    integer :: jnnz,nfrelem,nnew,status
    type(pdata_type), pointer :: pdata
...
! ******************************************************************
  ...
  subroutine tabline(n,x,m,p,lambda,f,csupn,ssupn,nlpsupn,bdsvio,xtype,outiter,totiter,nwcalls,nwtotit,pdataptr)
    ...
    write(20,8000) pdata%n_orig,pdata%m_orig,m,pdata%m_orig-m-ncdouble,ncdouble,n,m,p,pdata%nbds, &
         f,csupn,ssupn,nlpsupn,bdsvio,xtype,maxval(abs(x(1:n))),maxval(abs(lambda(1:m+p))),outiter, &
         totiter,nwcalls,nwtotit,pdata%counters(1:5)
    close(20)

8000 format(9(1X,I6),1X,1P,D24.16,4(1X,1P,D7.1),1X,I1,2(1X,1P,D7.1),9(1X,I8))
